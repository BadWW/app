...
var addTabs = function (options) {
    ...
    if (!$("#" + id)[0]) {
        ...
        //创建新TAB的title
        var title = '<li role="presentation" id="tab_' + id + '"><a href="#' + id + '" aria-controls="' + id +
            '" role="tab" data-toggle="tab"><i class="'+options.icon+'"></i>' + options.title;
        //是否允许关闭
        if (options.close) {
        ...
        if (options.content) {
            ...
            content = '<div role="tabpanel" class="tab-pane" id="' + id + '"><iframe id="iframe_'+id+'" src="' + options.url +
                '" width="100%" height="100%" onload="changeFrameHeight(this)" frameborder="no" border="0" ' +
                'marginwidth="0" marginheight="0" scrolling="yes" allowtransparency="yes"></iframe></div>';
        }
        //加入TABS
...
var changeFrameHeight = function (that) {
    ...
    $(that).parent(".tab-pane").height(document.documentElement.clientHeight - 235);
};
